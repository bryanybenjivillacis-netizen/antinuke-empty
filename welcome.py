...
def _resolve_channel_link(url: str, guild: discord.Guild) -> str:
    """Si el 'url' del botón es una mención de canal (<#id>) o solo un ID, lo convierte
    en el link completo de Discord (https://discord.com/channels/guild_id/channel_id)."""
    raw = url.strip()
    m = re.match(r"<#(\d+)>$", raw)
    if not m:
        m = re.match(r"(\d{15,25})$", raw)
    if m:
        return f"https://discord.com/channels/{guild.id}/{m.group(1)}"
    return url
...
def _build_embed(entry: dict, member: discord.Member) -> tuple[discord.Embed, str, list[discord.ui.Button]]:
    ...
    for btn in entry.get("buttons", []):
        ...
        url = _resolve_channel_link(url, member.guild)
